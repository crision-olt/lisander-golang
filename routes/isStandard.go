...
/*IsStandard used to check if can update the date from where gets the notifications and publications.*/
func IsStandard(w http.ResponseWriter, r *http.Request) {
	status, err := database.IsStandard(UserID)
	if err != nil {
